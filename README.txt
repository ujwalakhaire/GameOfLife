...
OVERVIEW
GameOfLife is a console application which consists of board of live and dead cells. Board is constructed using data present in the input sample files. Input sample file can only contain x (live cells), - (dead cells) and white spaces (which are ignored.). GameController mediates input from file and commands board configuration and generation of ticks and also notify formatters to display output of the ticks.

TO BUILD & RUN THE APPLICATION
1. git clone git@github.com:ujwalakhaire/GameOfLife.git
2. Open it in VS IDE 2010.
3. Build the solution.
4. Navigate to Release folder in the GameOfLife project ( bin -> release)
5. Open command prompt.
6. Drag and drop GameOfLife.exe on command prompt.
7. Provide file path for an example pattern-file as an argument to the exe.
...
8. Hit Enter to see the input.
9. As mentioned on the console, press any key to see the next generation and so forth.
...
2. Unit tests are written to test board and cell's methods.
3. Open NUnit-x86.exe (On win-7, default location is C:\Program Files (x86)\NUnit 2.6\bin\ )
